...
const DEFAULT_PASSWORD = process.env.DEFAULT_REFERENT_ADMIN_PASSWORD || "brouette2026";
...
async function main() {
  ...
  if (!targets.length) {
    console.log("Aucun membre admin/referent trouve.");
    return;
  }
  ...
  console.log(`Comptes a traiter: ${targets.length}`);
  ...
  for (const { id, data } of targets) {
    ...
    console.log(`OK ${role.toUpperCase()} ${id} -> ${primaryEmail}`);
  }
